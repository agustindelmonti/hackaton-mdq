...
// Utilidades genéricas, stock derivado, cálculo de discrepancia y la
// redacción de hipótesis viven ahora en logic.js (compartido con el
// prototipo desktop) — ver ese archivo para escapeHtml, fmtKg,
// normaliza, fechaCorta, diasDesde, stockPorLoteUbic,
// stockTotalUbicacion, lotesConStockEn, ubicacionActualDeLote,
// stockTotalLote, movimientosDeLote, ultimoIngresoA,
// calcularDiscrepancia, redactarHipotesisMerma, candidatosDiscrepancia.

// ---------- Resolución determinística de identificadores (fuzzy) ----------
// Específico del prototipo mobile: sólo la captura por voz necesita
// fuzzy-match de texto dictado contra el catálogo.
...
// redactarHipotesisMerma y calcularDiscrepancia viven en logic.js
// (compartidas con el prototipo desktop) — ver ese archivo para el
// comentario largo sobre cómo esto se conecta con un LLM real.
...
function renderDiscrepancia() {
  ...
  if (isNaN(kgContado)) {
    ...
    if (!excede) {
      ...
      // Capa determinística: filtra candidatos ANTES de narrar (ver
      // candidatosDiscrepancia en logic.js). La hipótesis
      // (redactarHipotesisMerma, también en logic.js) sólo puede hablar
      // sobre lo que está en esta lista acotada — nunca sobre toda la
      // tabla de movimientos.
      const candidatos = candidatosDiscrepancia(loteId);
