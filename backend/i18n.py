...
CATALOGO: dict[str, dict[str, str]] = {
    ...
    "core.opn.analisis_p3": {"es": "{n} de esos lotes ya tienen una orden de carga abierta esperándolos: ahí el vencimiento no es sólo un plazo, es plata comprometida que se frena.",
     "en": "{n} of those lots already have an open load order waiting on them: there the deadline isn't just administrative, it's committed money that stalls."},
    "core.opn.analisis_i": {"es": "análisis de hace {dias} días",
     "en": "analysis {dias} days old"},
    "core.opn.analisis_i_comprometido": {"es": "análisis de hace {dias} días — comprometido en {ordenes}",
     "en": "analysis {dias} days old — committed to {ordenes}"},
